...
SCENES = V2 / "scenes" / "SUNDAY_SCENE_PROGRAM.json"
MANIFEST = V2 / "audio" / "SUNDAY_AUDIO_MANIFEST.json"


class SevilleV2SundayAudioProductionTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_exact_primary_cardinality_and_optional_depth_boundary(self):
        self.assertEqual(9, len(self.primary))
        self.assertEqual(9, len(self.entries))
        self.assertEqual(4, len(self.scene_program["optional_depth"]))
        ...
        audio_programs = sorted((V2 / "audio").glob("SEV2-SUN-*.json"))
        self.assertEqual(9, len(audio_programs))
